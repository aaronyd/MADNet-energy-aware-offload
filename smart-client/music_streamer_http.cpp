...
//open file as temp/xxx.mp3
  ...
  if (current_l == 0) {
    ...
    fprintf(fp ,"total time = %f seconds; total length = %f bytes; throughput of this song = %f Mbps\n",
...
int do_wifi_download_http(char *interface, char *essid, char *mac_str)
{
  int index;
  int done = 0;
  char url[80];
...
//      }
      ...
      sprintf(url, "http://192.168.1.1:8000/%d.mp3", playlist[index]);
      sprintf(filename, "temp/%d.mp3", playlist[index]);

      if (http_download_file(url, filename, &tag)==1) {
        music_location[index] = 1;
      }
      else {
#if 0
...
//      do_wifi_download(WIFI_ITFACE, essid, mac_str);
      do_wifi_download_http(WIFI_ITFACE, essid, mac_str);
    }
    else {
